package tk.luminos.luminoscore.graphics;
...
public class FrameBufferObject {
	...
	private boolean useMultisamples = false;
	...
	public FrameBufferObject(int width, int height, int depthBufferType, boolean useMultisample) {
		this.width = width;
		this.height = height;
		this.useMultisamples = useMultisample;
		initialiseFrameBuffer(depthBufferType);
	}
	...
	/**
	 * Gets the GPU index of the color buffer
	 * 
	...
	/**
	 * Gets the GPU index of the depth buffer
	 * 
	...
	public void resolveToFBO(FrameBufferObject fboOutput) {
		GL30.glBindFramebuffer(GL30.GL_DRAW_FRAMEBUFFER, fboOutput.frameBuffer);
		GL30.glBindFramebuffer(GL30.GL_READ_FRAMEBUFFER, this.frameBuffer);
		GL30.glBlitFramebuffer(0, 0, width, height, 0, 0, fboOutput.width, fboOutput.height, GL11.GL_COLOR_BUFFER_BIT | GL11.GL_DEPTH_BUFFER_BIT, GL11.GL_NEAREST);
	...
	/**
	 * Creates the FBO along with a color buffer texture attachment
	 * 
	...
	/**
	 * Adds a depth buffer to the FBO in the form of a texture
	 */
	...
	/**
	 * Adds a multisampled color attachment to the FBO
	 */
