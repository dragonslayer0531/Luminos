...
/**
 * 
 * Serialization Utilities for Networking and Object Importing
 * 
 * @author Nick Clark
 * @version 1.0
 *
...
public class SerializationUtils {
	...
	public static int writeBytes(byte[] dest, int pointer, char src) {
		...
		dest[pointer++] = (byte)((src >> 8) &0xff);
		dest[pointer++] = (byte)((src >> 0) &0xff);
		return pointer;
	}
	...
	public static int writeBytes(byte[] dest, int pointer, int src) {
		...
		dest[pointer++] = (byte) ((src >> 24) & 0xff);
		dest[pointer++] = (byte) ((src >> 16) & 0xff);
		dest[pointer++] = (byte) ((src >> 8) & 0xff);
		dest[pointer++] = (byte) ((src >> 0) & 0xff);
		return pointer;
	}
	...
	public static int writeBytes(byte[] dest, int pointer, long src) {
		...
		dest[pointer++] = (byte) ((src >> 56) & 0xff);
		dest[pointer++] = (byte) ((src >> 48) & 0xff);
		dest[pointer++] = (byte) ((src >> 40) & 0xff);
		dest[pointer++] = (byte) ((src >> 32) & 0xff);
		dest[pointer++] = (byte) ((src >> 24) & 0xff);
		dest[pointer++] = (byte) ((src >> 16) & 0xff);
		dest[pointer++] = (byte) ((src >> 8) & 0xff);
		dest[pointer++] = (byte) ((src >> 0) & 0xff);
	...
	public static int writeBytes(byte[] dest, int pointer, double src) {
		...
		long data = Double.doubleToLongBits(src);
		return writeBytes(dest, pointer, data);
	}
	
	public static int writeBytes(byte[] dest, int pointer, boolean src) {
		assert(dest.length > pointer + Type.getSize(Type.BOOLEAN));
		dest[pointer] = (byte)(src ? 1 : 0);
		return pointer;
	}
	...
	public static long readLong(byte[] src, int pointer) {
		return ByteBuffer.wrap(src, pointer, 8).getInt();
	}
