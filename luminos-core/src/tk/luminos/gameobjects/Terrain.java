...
import tk.luminos.serialization.*;
...
public class Terrain implements Serializable<DBObject> {
    ...
    public Terrain(float gridX, float gridZ, int seed, String[] textures) {
        this.textures = textures;
        try {
            this.texturePack = new TerrainTexturePack(new TerrainTexture(Loader.getInstance().loadTexture(textures[0])),
                    new TerrainTexture(Loader.getInstance().loadTexture(textures[1])),
                    new TerrainTexture(Loader.getInstance().loadTexture(textures[2])),
                    new TerrainTexture(Loader.getInstance().loadTexture(textures[3])));
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public Terrain(float x, float z, float[] vertices, float[] textureCoords, int[] indices, String[] textures) {
        this.x = x;
        this.z = z;

        this.model = Loader.getInstance().load(vertices, textureCoords, indices, 3);
        this.heights = new float[VERTEX_COUNT][VERTEX_COUNT];
        for (int x_in = 0; x_in < VERTEX_COUNT; x_in++) {
            for (int y = 0; y < VERTEX_COUNT; y++) {
                heights[x_in][y] = vertices[3 * (x_in * VERTEX_COUNT + y) + 1];
            }
        }
    ...
    public Terrain(float x, float z, float[] vertices, float[] normals, float[] textureCoords, int[] indices, TerrainTexturePack ttp) {
        this.x = x;
        this.z = z;

        this.model = Loader.getInstance().load(vertices, textureCoords, normals, indices);
        this.heights = new float[VERTEX_COUNT][VERTEX_COUNT];
        for (int x_in = 0; x_in < VERTEX_COUNT; x_in++) {
            for (int y = 0; y < VERTEX_COUNT; y++) {
                heights[x_in][y] = vertices[3 * (x_in * VERTEX_COUNT + y) + 1];
    ...
    public void genBlendMap() {
        this.blendMap = new TerrainTexture(Loader.getInstance().loadTexture(ProceduralTerrainTexture.generateTerrainMap(this)));
    }

    public Terrain(float x, float z, float[] vertices, float[] textureCoords, int[] indices, TerrainTexturePack ttp, BufferedImage heightmap) {
        this.x = x;
        this.z = z;
        this.model = Loader.getInstance().load(vertices, textureCoords, indices, 3);
        this.heights = new float[VERTEX_COUNT][VERTEX_COUNT];
        ...
        this.blendMap = new TerrainTexture(Loader.getInstance().loadTexture(heightmap));
        this.texturePack = ttp;
    }
    ...
    public static Terrain deserialize(DBObject serializedTerrain) {
        float[] positions = serializedTerrain.findArray("position").floatData;
        float[] vertices = serializedTerrain.findArray("vertices").floatData;
        float[] normals = serializedTerrain.findArray("normals").floatData;
        float[] textureCoords = serializedTerrain.findArray("textureCoords").floatData;
        int[] indices = serializedTerrain.findArray("indices").intData;
        serializedTerrain.strings.stream().forEach(str -> System.out.println(str.getString()));
        String name = serializedTerrain.findString("ttp").getString();
        System.out.println(name);
        TerrainTexturePack ttp = AssetCache.findTerrainTexturePack(name);
        return new Terrain(positions[0], positions[2], vertices, textureCoords, normals, indices, ttp);
    }

    public DBObject serialize(String name) {
        DBObject obj = new DBObject(name, DBObjectType.TERRAIN);
        obj.addArray(new Vector3(x, 0, z).serialize("position"));
        obj.addArray(DBArray.createFloatArray("vertices", this.vertices));
        obj.addArray(DBArray.createFloatArray("textureCoords", this.textureCoords));
        obj.addArray(DBArray.createFloatArray("normals", this.normals));
        obj.addArray(DBArray.createIntegerArray("indices", this.indices));
        obj.addString(DBString.create("ttp", texturePack.name));
        return obj;
    }
